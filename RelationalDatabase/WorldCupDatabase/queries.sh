...
echo $($PSQL "SELECT SELECT ROUND(AVG(winner_goals), 2) FROM games")"

echo -e "\nAverage number of goals in all games from both teams:"
echo $($PSQL "SELECT AVG(winner_goals) AS average_winner_goals, AVG(opponent_goals) AS average_opponent_goals FROM games;")"

echo -e "\nMost goals scored in a single game by one team:"
echo $($PSQL "SELECT MAX(winner_goals) AS most_goals_in_single_games FROM games")"

echo -e "\nNumber of games where the winning team scored more than two goals:"
echo $($PSQL "SELECT * FROM games WHERE winner_goals > 2")"
